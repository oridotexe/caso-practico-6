...
CREATE TABLE RESUMEN_COMISIONES_VENDEDORES (
    ...
    VENDEDOR            VARCHAR2(50 CHAR) NOT NULL,
...
ALTER TABLE RESUMEN_COMISIONES_VENDEDORES ADD CONSTRAINT PK_RESUMEN_COMISIONES PRIMARY KEY (AÑO,MES,VENDEDOR);
...
CREATE OR REPLACE PROCEDURE CARGAR_COMISIONES_VENDEDORES(P_AÑO NUMBER, P_MES NUMBER) IS
    ...
    V_VENDEDOR VARCHAR2(50);

BEGIN

    IF NOT VALIDAR_AÑO_TRANSACCIONES(P_AÑO) THEN
        RAISE_APPLICATION_ERROR(-20200, 'ERROR: NO EXISTEN TRANSACCIONES PARA EL AÑO ESPECIFICADO.');
    END IF;

    IF NOT VALIDAR_MES_SIGUIENTE(P_AÑO, P_MES) THEN
        RAISE_APPLICATION_ERROR(-20201,'ERROR: EL MES DEBE SER EL SIGUIENTE AL ÚLTIMO PROCESADO.');
    END IF;
    ...
    FOR REC IN CUR_VENDEDORES LOOP
        ...
        WHERE F.FK_VENDEDORES = REC.ID_VENDEDOR
        AND EXTRACT(YEAR FROM C.FECHA_COBRO) = P_AÑO
        ...
        INSERT INTO RESUMEN_COMISIONES_VENDEDORES (
            ...
            V_MONTO_TOTAL_POR_COBRAR, 0.15, V_MONTO_COMISION 
        );
    END LOOP;
...
-- PARTE C: Calcular el vendedor con mayor comisión

CREATE OR REPLACE FUNCTION VENDEDOR_MAX_COMISION(P_AÑO NUMBER, P_MES NUMBER) RETURN VARCHAR2 IS
    V_VENDEDOR VARCHAR2(50);
    V_MONTO_COMISION_ACTUAL NUMBER(20,2);
    V_MONTO_COMISION_NUEVO NUMBER(20,2);
    V_MONTO_COBRADO NUMBER(20,2);
BEGIN

     SELECT VENDEDOR, MONTO_TOTAL_COBRADO, MONTO_COMISION
     INTO V_VENDEDOR, V_MONTO_COBRADO, V_MONTO_COMISION_ACTUAL
     FROM RESUMEN_COMISIONES_VENDEDORES
     WHERE AÑO = P_AÑO AND MES = P_MES
        AND MONTO_COMISION = (
        SELECT MAX(MONTO_COMISION)
        FROM RESUMEN_COMISIONES_VENDEDORES
        WHERE AÑO = P_AÑO AND MES = P_MES
        );

   V_MONTO_COMISION_NUEVO := V_MONTO_COMISION_ACTUAL + (V_MONTO_COBRADO * 0.05);

    UPDATE RESUMEN_COMISIONES_VENDEDORES
    SET MONTO_COMISION = V_MONTO_COMISION_NUEVO, PORCENTAJE_COMISION = 0.20
    WHERE AÑO = P_AÑO AND MES = P_MES AND VENDEDOR = V_VENDEDOR;
...
-- CARGADO AUTOMÁTICO DE DATOS EN LA TABLA DE COMISIONES

BEGIN
    FOR cursor_año IN (SELECT DISTINCT EXTRACT(YEAR FROM FECHA_FACTURA) año FROM FACTURAS ORDER BY año ASC) LOOP
        FOR cursor_mes IN (SELECT DISTINCT EXTRACT(MONTH FROM FECHA_FACTURA) mes FROM FACTURAS 
                            WHERE EXTRACT(YEAR FROM FECHA_FACTURA) = cursor_año.año ORDER BY mes ASC) LOOP
            CARGAR_COMISIONES_VENDEDORES(cursor_año.año, cursor_mes.mes);
        END LOOP;
    END LOOP;
END;
/

--EJEMPLO PARA FUNCION VENDEDOR_MAX_COMISION
...
BEGIN
    FOR cursor_año IN (SELECT DISTINCT EXTRACT(YEAR FROM FECHA_FACTURA) año FROM FACTURAS ORDER BY año ASC) LOOP
        FOR cursor_mes IN (SELECT DISTINCT EXTRACT(MONTH FROM FECHA_FACTURA) mes FROM FACTURAS 
            ...
            V_VENDEDOR := VENDEDOR_MAX_COMISION(cursor_año.año, cursor_mes.mes);
            DBMS_OUTPUT.PUT_LINE('El vendedor con mayor comisión para el mes ' || cursor_mes.mes || 
                                ' del año ' || cursor_año.año || ' es: ' || V_VENDEDOR);
        END LOOP;
    END LOOP;
END;
/

SELECT * FROM RESUMEN_COMISIONES_VENDEDORES;