...
CREATE OR REPLACE FUNCTION CALCULAR_COMISION(P_MONTO_COBRADO NUMBER, P_PORCENTAJE NUMBER) RETURN NUMBER IS
BEGIN
    RETURN P_MONTO_COBRADO * (P_PORCENTAJE / 100);
END;
...
CREATE OR REPLACE FUNCTION VALIDAR_MES_SIGUIENTE(P_AÑO NUMBER, P_MES NUMBER) RETURN BOOLEAN IS
    ...
    V_ULTIMO_MES  NUMBER;
BEGIN
    SELECT NVL(MAX(AÑO), 0), NVL(MAX(MES), 0) INTO V_ULTIMO_AÑO, V_ULTIMO_MES 
    FROM RESUMEN_COMISIONES_VENDEDORES;

    IF V_ULTIMO_AÑO = 0 THEN
        RETURN TRUE;  
    ELSIF V_ULTIMO_AÑO = P_AÑO AND V_ULTIMO_MES + 1 = P_MES THEN
        RETURN TRUE;
    ELSIF V_ULTIMO_AÑO + 1 = P_AÑO AND P_MES = 1 THEN
        RETURN TRUE;
    END IF;
...
CREATE OR REPLACE PROCEDURE CARGAR_COMISIONES_VENDEDORES(P_AÑO NUMBER, P_MES NUMBER, P_PORCENTAJE NUMBER DEFAULT 15) IS
    CURSOR CUR_VENDEDORES IS
        SELECT V.ID_VENDEDOR, V.VENDEDOR
        FROM VENDEDORES V
        WHERE EXISTS (
            SELECT 1 
            FROM FACTURAS F 
            WHERE F.FK_VENDEDORES = V.ID_VENDEDOR 
...
BEGIN
    ...
    FOR REC IN CUR_VENDEDORES LOOP
        ...
        SELECT COUNT(*), NVL(SUM(F.TOTAL_FACTURA), 0) INTO V_CANT_FACTURAS, V_MONTO_TOTAL_FACT
        FROM FACTURAS F
        WHERE F.FK_VENDEDORES = REC.ID_VENDEDOR
        AND EXTRACT(YEAR FROM F.FECHA_FACTURA) = P_AÑO
        AND EXTRACT(MONTH FROM F.FECHA_FACTURA) = P_MES;

        SELECT NVL(SUM(C.VALOR_COBRADO), 0) INTO V_MONTO_TOTAL_COBRADO
        FROM COBRANZAS C
        ...
        V_MONTO_COMISION := CALCULAR_COMISION(V_MONTO_TOTAL_COBRADO, P_PORCENTAJE);
        ...
        INSERT INTO RESUMEN_COMISIONES_VENDEDORES (
            ...
            V_MONTO_TOTAL_POR_COBRAR, P_PORCENTAJE, V_MONTO_COMISION
        );
    END LOOP;
