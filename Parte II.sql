...
BEGIN
     ...
     SELECT VENDEDOR, MONTO_COMISION
     INTO V_VENDEDOR, V_MONTO_COMISION_ACTUAL
     FROM RESUMEN_COMISIONES_VENDEDORES
   ...
   V_MONTO_COMISION_NUEVO := V_MONTO_COMISION_ACTUAL + (V_MONTO_COMISION_ACTUAL * 0.05);

    UPDATE RESUMEN_COMISIONES_VENDEDORES
    SET MONTO_COMISION = V_MONTO_COMISION_NUEVO
...
BEGIN
    ...
    IF NOT VALIDAR_AÑO_TRANSACCIONES(P_AÑO) THEN
        DBMS_OUTPUT.PUT_LINE('ERROR NO EXISTEN TRANSACCIONES PARA EL AÑO ESPECIFICADO.');
    END IF;

    IF NOT VALIDAR_MES_SIGUIENTE(P_AÑO, P_MES) THEN
        DBMS_OUTPUT.PUT_LINE('ERROR EL MES DEBE SER EL SIGUIENTE AL ÚLTIMO PROCESADO.');
    END IF;
